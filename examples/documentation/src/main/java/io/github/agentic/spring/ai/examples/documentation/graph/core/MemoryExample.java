 ...
 */
package io.github.agentic.spring.ai.examples.documentation.graph.core;
...
public class MemoryExample {
	...
	public static void longTermMemoryWithDatabase() throws GraphStateException {
		// 在节点中使用 Store 存储用户信息
		var userProfileNode = io.github.agentic.spring.ai.graph.action.AsyncNodeActionWithConfig.node_async((state, config) -> {
	...
	public static void longTermMemoryWithRedis() throws GraphStateException {
		var cacheNode = io.github.agentic.spring.ai.graph.action.AsyncNodeActionWithConfig.node_async((state, config) -> {
	...
	public static void combinedMemoryExample(ChatClient.Builder chatClientBuilder) throws GraphStateException {
		...
		// 加载用户偏好（长期内存）
		var loadUserPreferences = io.github.agentic.spring.ai.graph.action.AsyncNodeActionWithConfig.node_async((state, config) -> {
			String userId = (String) state.value("userId").orElse("");
