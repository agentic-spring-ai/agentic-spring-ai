...
Modify the model configuration in `agentic-spring-ai-admin-server/model-config.yaml` according to your model provider.
...
If you need to modify the Nacos address, please update the configuration in the `agentic-spring-ai-admin-server/src/main/resources/application.yml` file
```yaml
nacos:
...
sh start.sh
```
Start the application in the agentic-spring-ai-admin-server directory
```bash
mvn spring-boot:run
```
