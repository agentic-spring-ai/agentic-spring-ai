...
export function ToolRequestConfirmMessage({ message }: { message: UIMessage }) {
  // Verify it's a tool request confirm message
  if (!isToolRequestConfirmMessage(message.message)) {
    console.warn(
      "[ToolRequestConfirmMessage] Not a tool request confirm message:",
      message,
    );
    return null;
  }
  ...
  if (toolFeedback.length === 0 && toolsAutomaticallyApproved.length === 0) {
    console.warn(
      "[ToolRequestConfirmMessage] No tool feedback or auto-approved tools found",
    );
    return null;
  }
