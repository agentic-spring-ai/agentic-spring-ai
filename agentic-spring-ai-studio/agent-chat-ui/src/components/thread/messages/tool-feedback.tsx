...
function AutomaticallyApprovedToolItem({
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-green-200 bg-white p-3">
...
function ToolFeedbackItem({
  ...
  onUpdate: (
    id: string,
    result: "APPROVED" | "REJECTED" | "EDITED" | null,
    editedArgs?: string,
  ) => void;
  disabled: boolean;
}) {
  ...
  const [localResult, setLocalResult] = useState<
    "APPROVED" | "REJECTED" | "EDITED" | undefined
  >(feedback.result);
  ...
  const borderClass = localResult
    ? resultStyles[localResult]
...
export function ToolFeedbackConfirm({ message }: { message: UIMessage }) {
  ...
  const [feedbackState, setFeedbackState] = useState<
    Record<
      string,
      { result: "APPROVED" | "REJECTED" | "EDITED"; editedArgs?: string }
    >
  >({});
  ...
  if (!isToolRequestConfirmMessage(message.message)) {
    console.warn(
      "[ToolFeedbackConfirm] Not a tool request confirm message:",
      message,
    );
    return null;
  }
  ...
  const toolsAutomaticallyApproved =
    message.message.toolsAutomaticallyApproved || [];
  ...
  if (toolFeedback.length === 0 && toolsAutomaticallyApproved.length === 0) {
    console.warn(
      "[ToolFeedbackConfirm] No tool feedback or auto-approved tools found",
    );
    return null;
  }
  ...
  const handleSubmitFeedback = async () => {
    ...
    const allFeedbackProvided = toolFeedback.every(
      (tf) => feedbackState[tf.id],
    );
    ...
    try {
      ...
      const feedbackForApi = toolFeedback.map((tf) => {
        ...
        return {
          ...
          name: tf.name,
          arguments:
            feedback.result === "EDITED" && feedback.editedArgs
              ? feedback.editedArgs
              : tf.arguments,
  ...
  const pendingCount = toolFeedback.filter(
    (tf) => !feedbackState[tf.id],
  ).length;
  ...
  return (
    <div className="group flex flex-col gap-3">
      ...
      >
        ...
        {toolsAutomaticallyApproved.length > 0 && (
          <div className="mb-3 rounded border border-blue-200 bg-blue-50 p-2">
            <div className="flex items-center gap-2 text-xs text-blue-800">
              ...
              <span>
                {toolsAutomaticallyApproved.length} tool
                {toolsAutomaticallyApproved.length > 1 ? "s" : ""} automatically
                approved
                {toolFeedback.length > 0 &&
                  `, ${toolFeedback.length} tool${toolFeedback.length > 1 ? "s" : ""} require${toolFeedback.length > 1 ? "" : "s"} confirmation`}
              </span>
            </div>
          </div>
        )}
        ...
        {!isSubmitted && !isSubmitting && toolFeedback.length > 1 && (
          <div className="mb-3 rounded border border-yellow-200 bg-yellow-100 p-2">
            <p className="mb-1 text-xs text-yellow-800">
              Please provide feedback for each tool call below. All feedback
              will be submitted together when complete.
            </p>
            {pendingCount > 0 && (
              ...
              <div className="text-xs text-gray-600">
                {allFeedbackProvided ? (
                  <span className="font-medium text-green-600">
                    ✓ All tool calls have feedback
                  </span>
                ) : (
                  <span>
                    Please provide feedback for all {toolFeedback.length} tool
                    call{toolFeedback.length > 1 ? "s" : ""} before submitting
                  </span>
                )}
              </div>
            )}
        ...
        {!hasAnyFeedback && !isSubmitted && (
          <p className="text-center text-xs text-yellow-600 italic">
            Please review each tool and provide your feedback (Approve, Reject,
            or Edit)
          </p>
        )}
      </Card>
