  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <main className="mx-auto max-w-5xl px-4 py-12">
        ...
        {isEmpty && !isLoading && (
          <div className="border-muted-foreground/30 bg-muted/30 rounded-xl border border-dashed p-12 text-center">
            <p className="text-muted-foreground">
              No agents or graphs available. Configure your application to
              expose agents or graphs.
            </p>
          </div>
        )}
              ...
              <section>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {agentList.map((name) => (
                    <Link
                      key={name}
                      href={`/index.html?agent=${encodeURIComponent(name)}`}
                    >
              ...
              <section>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {graphList.map((name) => (
                    <Link
                      key={name}
                      href={`/index.html?graph=${encodeURIComponent(name)}`}
                    >
                      <Card className="hover:border-primary cursor-pointer transition-all hover:shadow-md">
                        <CardHeader className="pb-2">
                          <CardTitle className="text-base">{name}</CardTitle>
                          <CardDescription>
                            Run this workflow graph
                          </CardDescription>
                        </CardHeader>
                        <CardContent className="pt-0">
                          <Button
